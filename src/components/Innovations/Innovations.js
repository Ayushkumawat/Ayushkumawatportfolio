...
export default function Innovations({ isActive, changeSection, currentSection }) {
  ...
  useEffect(() => {
    if (!isActive || isMobile || typeof changeSection !== 'function') return;
  ...
  }, [isActive, isMobile, changeSection, currentSection]);

  if (isMobile) {
    return <InnovationsMobile projects={projects} isActive={isActive} />;
  }

  return (
    <section
      className={`innovations-section${isActive ? ' innovations-active' : ''}`}
      ref={innovationsRef}
    >
